...
public class LinkedBinaryTree<T extends Comparable<T>>
    ...
    private boolean isBST(BTNode<T> node) {
        ...
        if ((node.getLeftChild() != null)
            ...
            return ((node.getElement().compareTo(node.getLeftChild().getElement()) > 0)
                    && (node.getElement().compareTo(node.getRightChild().getElement()) < 0)
        ...
        if (node.getRightChild() != null)
            return ((node.getElement().compareTo(node.getRightChild().getElement()) < 0)
                    && isBST(node.getRightChild()));
